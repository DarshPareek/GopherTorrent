...
parser = argparse.ArgumentParser(
    description="Calculate and/or compare .torrent infohash values"
)
parser.add_argument(
    "-o",
    choices=["calculate", "compare", "dedup"],
    required=True,
...
parser.add_argument(
    ...
    "--path",
    required=True,
    help="Path to .torrent files. For compare include list.txt in same directory as the script",
)
...
if args.o == "calculate":
    ...
    try:
        with io.open(script_dir + "list.txt", encoding="utf8") as f:
            lines = (
                f.read().splitlines()
            )  # Insert txt values from list.txt in array. If zero values error. if values compare both arrays. Similar in a txt list.
        if len(lines) > 0:
